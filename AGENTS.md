...
Do not place an OCR-unavailable or scanned-page-detected banner over the editor document canvas. Scanned pages should remain visually unobstructed; expose OCR through its dedicated workflow instead.

Document-opening transitions use the same minimal white loading surface as authentication: the official PDFEnrich document mark and a thin 72px progress line, centered on desktop and mobile, with a nonvisual status label and reduced-motion fallback. Do not show the recovery illustration, warning badge, large heading, or explanatory copy while a document loads; reserve recovery content and retry actions for actual errors.
