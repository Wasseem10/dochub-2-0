...
The selected July 29, 2026 Popular Tools redesign is Professional Document Commands (ideation option 2). `work/design-references/popular-tools-icons-exact-2026-07-29.png` supersedes earlier icon approximations and is the exact source for all ten tool glyphs. Use the individually extracted, transparent, square source assets in `runtime-public/tool-icons/`; keep their navy document outlines and PDFEnrich-blue command details unchanged, use `object-fit: contain`, and never redraw, stretch, warp, or substitute library glyphs for them.
