...
import { sanitizeDownloadFileName, sanitizePdfDisplayName } from "./tools/safeFileName.js";
import {
...
function downloadBlob(blob, name, toolId = "edit-pdf") {
  ...
  anchor.href = url;
  anchor.download = sanitizeDownloadFileName(name);
...
export function App({ view = "landing", appSection = "Home", authMode = "login", documentId = "", publicTool = "" }) {
  ...
  const [isPagesCollapsed, setIsPagesCollapsed] = useState(usesMobileEditorLayout);
  ...
  }, [activeDocumentId, editorRouteState, pages.length, publicTool]);
          ...
          <div className={`thumbnail-list ${viewMode}`}>
            {pages.map((page, index) => (
              ...
              <button
                ...
                onClick={() => {
                  setPageIndex(index);
                  if (usesMobileEditorLayout() && !isManagePagesOpen) setIsPagesCollapsed(true);
                }}
