...
export function FeaturesPage() {
  ...
  return (
    <main className="features-page">
      <PageMetadata
        title="All Features | PDFEnrich"
        description={`See all ${releasedTools.length} released PDFEnrich features for editing, organizing, converting, signing, scanning, protecting, and reviewing PDFs.`}
        canonicalUrl={ROUTE_PATHS.features}
        schemas={[{ "@context": "https://schema.org", "@type": "CollectionPage", name: "PDFEnrich features", url: absoluteSiteUrl(ROUTE_PATHS.features), mainEntity: { "@type": "ItemList", numberOfItems: releasedTools.length, itemListElement: releasedTools.map((tool, index) => ({ "@type": "ListItem", position: index + 1, name: tool.name, url: absoluteSiteUrl(tool.route) })) } }]}
      />
      ...
      <section className="features-hero">
        <div>
          ...
          <p>Explore the complete PDFEnrich workspace—from text editing and page organization to conversion, OCR, signing, protection, comparison, and export.</p>
      ...
      <section className="features-promises" aria-label="PDFEnrich product promises">
        ...
        <div><FileText size={21} /><span><strong>No PDFEnrich watermark</strong><small>Your downloaded document remains yours</small></span></div>
      </section>
      ...
      <section className="features-catalog" aria-labelledby="all-features-title">
        <header>
          <div><span>Complete capability list</span><h2 id="all-features-title">All working PDFEnrich features</h2><p>Only released, beta, or clearly limited workflows appear here. Each feature links to the real tool and shows its current availability.</p></div>
