...
function SinglePdfWorkspace({ tool }) {
  ...
  const isDelete = tool.id === "delete-pdf-pages";
  ...
  const exportResult = async () => {
    ...
    try {
      ...
      if (isSplit) {
        ...
        const outputSuffix = isDelete ? "pages-removed" : isRotate ? "rotated" : "organized";
        downloadBytes(await buildPdfFromPagePlan(sourceBytes, plan, `${tool.name} output`), "application/pdf", `${baseName}-${outputSuffix}.pdf`);
      }
  ...
  const actionCopy = isDelete
    ? { dropLabel: "Drop a PDF to delete pages", eyebrow: "Page removal", heading: "Choose pages to remove", help: "Delete only the pages you do not need. Undo restores the most recently removed page.", downloadLabel: "Download PDF with pages removed" }
    : isRotate
      ? { dropLabel: "Drop a PDF to rotate pages", eyebrow: "Page rotation", heading: "Rotate individual pages", help: "Rotate any page clockwise in 90-degree steps. Undo restores the previous rotation.", downloadLabel: "Download rotated PDF" }
      : { dropLabel: "Drop a PDF to organize", eyebrow: "Page organizer", heading: "Arrange the final PDF", help: "Drag pages to reorder. Rotate, duplicate, or delete them. Undo restores the previous organizer action.", downloadLabel: "Download organized PDF" };
  return <div className="conversion-workspace-grid"><section><PdfDropzone multiple={false} label={isSplit ? "Drop a PDF to split" : isExtract ? "Drop a PDF to extract pages" : actionCopy.dropLabel} onFiles={loadPdf} disabled={status === "reading" || status === "working"} />{status === "reading" && <div className="conversion-progress"><LoaderCircle className="is-spinning" size={18} /> Reading pages... {progress}%</div>}{error && <div className="conversion-error" role="alert">{error}</div>}{pages.length > 0 && <div className="page-organizer"><header><div><strong>{file.name}</strong><small>{pages.length} original page{pages.length === 1 ? "" : "s"} · {formatBytes(file.size)}</small></div>{isExtract ? <button type="button" onClick={() => setSelected(allSelected ? new Set() : new Set(pages.map((page) => page.sourceIndex)))}>{allSelected ? "Clear all" : "Select all"}</button> : !isSplit && <button type="button" disabled={!history.length} onClick={undo}><Undo2 size={16} /> Undo</button>}</header>{isSplit ? <div className="split-range-guide"><strong>Previewed pages</strong><div>{pages.map((page) => <span key={page.sourceIndex}><img src={page.previewUrl} alt={`Page ${page.sourceIndex + 1}`} /><small>{page.sourceIndex + 1}</small></span>)}</div></div> : isExtract ? <div className="extract-page-grid">{pages.map((page) => <label key={page.sourceIndex} className={selected.has(page.sourceIndex) ? "is-selected" : ""}><input type="checkbox" checked={selected.has(page.sourceIndex)} onChange={() => toggleSelected(page.sourceIndex)} /><img src={page.previewUrl} alt={`Page ${page.sourceIndex + 1}`} /><span>Page {page.sourceIndex + 1}</span></label>)}</div> : <div className="organizer-page-grid">{plan.map((item, index) => { const sourcePage = pages[item.sourceIndex]; return <article key={item.id} draggable={!isDelete && !isRotate} className={draggedIndex === index ? "is-dragging" : ""} onDragStart={() => setDraggedIndex(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggedIndex !== null) move(draggedIndex, index); setDraggedIndex(null); }} onDragEnd={() => setDraggedIndex(null)}><div className="organizer-page-preview"><img src={sourcePage.previewUrl} alt={`Output page ${index + 1}`} style={{ transform: `rotate(${item.rotation}deg)` }} /></div><strong>Page {index + 1}</strong><small>Original {item.sourceIndex + 1}{item.rotation ? ` · ${item.rotation}°` : ""}</small><footer>{!isDelete && !isRotate && <><button type="button" onClick={() => move(index, index - 1)} disabled={!index} aria-label="Move page left"><ArrowUp size={15} /></button><button type="button" onClick={() => move(index, index + 1)} disabled={index === plan.length - 1} aria-label="Move page right"><ArrowDown size={15} /></button></>}{!isDelete && <button type="button" onClick={() => rotate(item.id)} aria-label="Rotate page"><RotateCw size={15} /></button>}{!isDelete && !isRotate && <button type="button" onClick={() => duplicate(index)} aria-label="Duplicate page"><Copy size={15} /></button>}{!isRotate && <button type="button" onClick={() => remove(item.id)} disabled={plan.length <= 1} aria-label="Delete page"><Trash2 size={15} /></button>}</footer></article>; })}</div>}</div>}</section><aside className="conversion-settings-card"><span>{isSplit ? "Split ranges" : isExtract ? "Page selection" : actionCopy.eyebrow}</span><h2>{isSplit ? "Create separate PDFs" : isExtract ? "Extract selected pages" : actionCopy.heading}</h2>{isSplit && <label>Ranges<input className="page-range-input" value={ranges} onChange={(event) => setRanges(event.target.value)} placeholder="1-3, 4-6, 7" /><small>Each comma-separated range becomes a separate PDF.</small></label>}{!isSplit && !isExtract && <p className="page-tool-help">{actionCopy.help}</p>}<div className="conversion-summary" role="status" aria-live="polite"><Check size={18} /><span>{!pages.length ? "Upload a PDF to continue" : isSplit ? `${pages.length} pages ready` : isExtract ? `${selected.size} page${selected.size === 1 ? "" : "s"} selected` : `${plan.length} output page${plan.length === 1 ? "" : "s"}`}</span></div><button className="conversion-primary-action" type="button" disabled={!pages.length || status === "working" || (isExtract && !selected.size)} onClick={exportResult}>{status === "working" ? <><LoaderCircle className="is-spinning" size={18} /> Processing...</> : <><Download size={18} /> {isSplit ? "Download split files" : isExtract ? "Download extracted PDF" : actionCopy.downloadLabel}</>}</button>{status === "complete" && <ExportSuccessState toolId={tool.id} onDownloadAgain={exportResult} onStartAnother={() => { setFile(null); setSourceBytes(null); setPages([]); setPlan([]); setSelected(new Set()); setStatus("idle"); }} relatedRoute="/merge-pdf" relatedName="Merge PDF" />}</aside></div>;
}
