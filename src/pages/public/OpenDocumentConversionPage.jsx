...
export function OpenDocumentConversionPage({ tool }) {
  ...
  const convert = async () => {
    ...
    try {
      const baseName = file.name.replace(/\.[^.]+$/, "") || "pdfenrich-document";
  ...
  return <main className="image-conversion-page office-conversion-page open-document-conversion-page">
    ...
    </div>{status === "reading" && <div className="conversion-progress"><LoaderCircle className="is-spinning" size={18} /> Reading the {mode.label} file…</div>}<WorkflowErrorState message={error} onDismiss={() => setError("")} onRetry={file && status === "ready" ? convert : undefined} />{file && <div className="office-file-card"><header><ModeIcon size={20} /><div><strong>{file.name}</strong><small>{formatBytes(file.size)} · ready for conversion</small></div></header></div>}</section>
