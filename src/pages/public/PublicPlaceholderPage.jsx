...
const pageContent = {
  [ROUTE_PATHS.privacy]: {
    ...
    sections: [
      ["Browser processing", "The editor and supported merge, split, organize, rotate, page-removal, PDF-to-image, and image-to-PDF tools process supported files in your browser. Files are not sent to PDFEnrich merely to run these tools."],
      ["Optional Firebase account features", "Signing in uses Firebase Authentication. An administrator with the required Firebase custom claim can review the account name, email, sign-in method, and last sign-in time. If the authenticated private-cloud API is configured, an account holder can explicitly save a finished PDF and minimal version metadata for cross-device access. Signing in never uploads browser-local documents, workspace state, OCR text, thumbnails, or the signature library."],
  ...
  [ROUTE_PATHS.security]: {
    intro: "PDFEnrich separates browser-only PDF work from optional account and cloud features, and avoids claiming safeguards that have not been verified.",
    sections: [
  ...
  [ROUTE_PATHS.help]: {
    ...
    sections: [
      ...
      ["Download", "Use the editor or tool's download action. PDFEnrich does not add a watermark and does not require signup before a supported download."],
  ...
  [ROUTE_PATHS.terms]: {
    ...
    sections: [
      ...
      ["No professional advice", "PDFEnrich is a document tool, not legal, medical, financial, tax, or filing advice. Permanent redaction and other high-stakes outputs must be independently reviewed before use."],
      ["Account termination", "You may delete your account from Settings. PDFEnrich may restrict abusive or unlawful use needed to protect the service and other users."],
  ...
  [ROUTE_PATHS.dataRetention]: {
    intro: "What PDFEnrich stores today, how long it remains, and the controls available to remove it.",
    sections: [
      ...
      ["Account and cloud documents", "Firebase Authentication keeps the account until you delete it. A private cloud PDF exists only after you explicitly save that document. Account deletion requires recent authentication and confirms removal of active and legacy Firestore/Storage data before removing the sign-in identity; if the cloud purge fails, PDFEnrich stops instead of reporting a partial deletion as complete."],
      ...
      ["Deletion limitations", "Browser storage, browser backups, Firebase operational backups, and previously downloaded PDFs may follow separate technical lifecycles. Deleting a PDFEnrich account cannot remove a file already downloaded to a device or shared elsewhere."],
