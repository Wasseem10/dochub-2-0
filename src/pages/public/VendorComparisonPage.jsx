...
export function VendorComparisonPage() {
  ...
  return (
    <main className="vendor-comparison-page" style={{ "--comparison-accent": comparison.accent }}>
      <PageMetadata
        title={`${comparison.seoTitle} | PDFEnrich`}
      ...
      <section className="comparison-hero is-detail">
        ...
        <div className="comparison-hero-marks"><span className="is-pdfenrich">PE</span><b>vs</b><span>{comparison.mark}</span></div>
        <h1>PDFEnrich vs <em>{comparison.company}</em></h1>
        <p>{comparison.summary}</p>
        <div className="comparison-hero-actions">
          <Link to={ROUTE_PATHS.editPdf} onClick={() => trackComparisonCta(path, "hero_try")}>Try PDFEnrich <ArrowRight size={17} /></Link>
      ...
      <section className="comparison-quick-verdict" id="overview" aria-label="Quick recommendation">
        <article><small>CHOOSE PDFENRICH FOR</small><h2>{comparison.bestForPdfEnrich}</h2><p>{comparison.competitorFacts.processing.startsWith("Online") ? "Keep supported core tasks in the browser while staying free to begin as a guest." : "Start quickly with an account-optional browser editor and local processing where supported."}</p></article>
      ...
      <section className="comparison-table-section" id="comparison-table">
        ...
        <div className="comparison-table-wrap" tabIndex="0" role="region" aria-label={`Feature comparison: PDFEnrich and ${comparison.company}`}>
          <table>
            <thead><tr><th scope="col">Area</th><th scope="col">PDFEnrich</th><th scope="col">{comparison.company}</th></tr></thead>
            <tbody>{comparison.rows.map(([area, pdfEnrich, competitor]) => <tr key={area}><th scope="row">{area}</th><td>{pdfEnrich}</td><td>{competitor}</td></tr>)}</tbody>
          </table>
        </div>
      </section>

      <section className="comparison-reasons-section">
        <header><span>Why people choose PDFEnrich</span><h2>A lighter route through everyday PDF work.</h2><p>The strongest reason to switch is not a longer feature list. It is a workflow that better matches the job you actually need to finish.</p></header>
      ...
      <section className="comparison-plans-section" id="plans">
        ...
        <p className="comparison-table-hint">Swipe horizontally to compare all columns.</p>
        <div className="comparison-table-wrap" tabIndex="0" role="region" aria-label={`Plan comparison: PDFEnrich and ${comparison.company}`}>
          <table>
            <thead><tr><th scope="col">Plan question</th><th scope="col">PDFEnrich</th><th scope="col">{comparison.company}</th></tr></thead>
            <tbody>{planRows.map(([area, pdfEnrich, competitor]) => <tr key={area}><th scope="row">{area}</th><td>{pdfEnrich}</td><td>{competitor}</td></tr>)}</tbody>
          </table>
        </div>
      ...
      <section className="comparison-choice-grid">
        <article className="is-pdfenrich">
          <span><ShieldCheck size={20} /> PDFEnrich is likely the better fit when…</span>
          <ul>{comparison.pdfEnrichReasons.map((reason) => <li key={reason}><Check size={17} />{reason}</li>)}</ul>
      ...
      <section className="comparison-final-cta">
        ...
        <Link to={ROUTE_PATHS.editPdf} onClick={() => trackComparisonCta(path, "final_cta")}>Try PDFEnrich free <ArrowRight size={18} /></Link>
      </section>

      <section className="comparison-related">
        <header><span>Other comparisons</span><h2>Still deciding?</h2></header>
        <div>{otherComparisons.map((item) => <Link key={item.slug} to={comparisonPath(item.slug)}><span>PDFEnrich vs</span><strong>{item.company}</strong><ArrowRight size={16} /></Link>)}</div>
      </section>
      ...
      <p className="comparison-disclaimer">{comparison.company} and its product names are trademarks of their respective owners. PDFEnrich is not affiliated with or endorsed by {comparison.company}.</p>
    </main>
  );
}
