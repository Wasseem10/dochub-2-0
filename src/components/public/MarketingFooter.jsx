...
export function MarketingFooter() {
  return (
    <footer className="marketing-footer">
      <div className="marketing-footer-intro"><Link className="marketing-brand" to={ROUTE_PATHS.home} aria-label="PDFArrow home"><BrandWordmark logo /></Link><p>Edit PDFs online. Completely free, with no watermark or forced signup for supported tools.</p><Link className="marketing-primary" to={ROUTE_PATHS.editPdf}>Choose a PDF</Link></div>
      ...
      <div className="marketing-footer-bottom"><span>© 2026 PDFArrow</span><nav aria-label="Resources and legal"><Link to={ROUTE_PATHS.about}>About</Link><Link to={ROUTE_PATHS.compare}>Comparisons</Link><Link to={ROUTE_PATHS.resources}>Research & guides</Link><Link to={ROUTE_PATHS.templates}>Templates</Link><Link to={ROUTE_PATHS.privacy}>Privacy</Link><Link to={ROUTE_PATHS.security}>Security</Link><Link to={ROUTE_PATHS.terms}>Terms</Link><Link to={ROUTE_PATHS.dataRetention}>Data retention</Link><Link to={ROUTE_PATHS.support}>Support</Link></nav></div>
    </footer>
  );
}
