...
export function createXlsxFromPdfPages(pages, { title = "PDF export" } = {}) {
  ...
  files["docProps/core.xml"] = xml(`<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>${escapeXml(title)}</dc:title><dc:creator>PDFEnrich</dc:creator><cp:lastModifiedBy>PDFEnrich</cp:lastModifiedBy><dcterms:created xsi:type="dcterms:W3CDTF">${new Date(0).toISOString()}</dcterms:created></cp:coreProperties>`);
  files["docProps/app.xml"] = xml(`<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes"><Application>PDFEnrich</Application><Sheets>${sheets.length}</Sheets></Properties>`);
...
export function createStandaloneHtmlFromPdfPages(pages, { title = "PDF export" } = {}) {
  ...
  const body = pages.map((page, pageIndex) => {
    ...
    const text = (page.items || []).map((item) => `<text x="${Number(item.x || 0).toFixed(2)}" y="${Number(item.y || 0).toFixed(2)}" font-size="${Math.max(4, Number(item.fontSize || 10)).toFixed(2)}" font-family="${escapeHtml(item.fontFamily || "Arial, sans-serif")}">${escapeHtml(item.text)}</text>`).join("");
    const accessibleText = (page.items || []).map((item) => escapeHtml(item.text)).join(" ");
    const visual = page.dataUrl
      ? `<image href="${escapeHtml(page.dataUrl)}" x="0" y="0" width="${width}" height="${height}" preserveAspectRatio="none"/>`
      : `<rect width="100%" height="100%" fill="white"/>`;
    return `<section class="pdf-page" aria-label="Page ${pageIndex + 1}"><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="PDF page ${pageIndex + 1}" preserveAspectRatio="xMidYMid meet">${visual}<g class="selectable-text" aria-hidden="true">${text}</g></svg><p class="sr-only">${accessibleText}</p></section>`;
  }).join("\n");
  return `<!doctype html>\n<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>*{box-sizing:border-box}body{margin:0;padding:32px;background:#eef2f7;color:#111827;font-family:Arial,sans-serif}.pdf-document{display:grid;gap:24px;justify-items:center}.pdf-page{position:relative;width:min(100%,900px);background:#fff;box-shadow:0 12px 36px rgba(15,23,42,.16)}.pdf-page svg{display:block;width:100%;height:auto}.selectable-text text{fill:transparent;stroke:none;white-space:pre;user-select:text}.selectable-text text::selection{background:rgba(40,81,235,.28)}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}@media(max-width:640px){body{padding:12px}.pdf-document{gap:12px}}@media print{body{padding:0;background:#fff}.pdf-document{gap:0}.pdf-page{width:100%;box-shadow:none;break-after:page}}</style></head><body><main class="pdf-document" aria-label="${escapeHtml(title)}">${body}</main></body></html>`;
}
...
export async function createPptxFromRenderedPages(pages, { title = "PDF presentation" } = {}) {
  ...
  presentation.defineLayout({ name: "PDFENRICH_SOURCE", width: slideWidth, height: slideHeight });
  presentation.layout = "PDFENRICH_SOURCE";
  presentation.author = "PDFEnrich";
  ...
  presentation.company = "PDFEnrich";
