...
export function loadEditorSignatureLibrary(storage = typeof window !== "undefined" ? window.localStorage : null) {
  if (!storage) return [];
  try {
    const parsed = JSON.parse(storage.getItem(EDITOR_SIGNATURE_LIBRARY_KEY) || "[]");
...
export function persistEditorSignatureLibrary(signatures = [], storage = typeof window !== "undefined" ? window.localStorage : null) {
  if (!storage) return;
  const normalized = signatures.map(normalizeEditorSignature).filter(Boolean).slice(0, MAX_SAVED_EDITOR_SIGNATURES);
  try {
    storage.setItem(EDITOR_SIGNATURE_LIBRARY_KEY, JSON.stringify(normalized));
  } catch {
