...
const definitions = [
  ...
  ["compress-pdf", "Compress PDF", "Reduce PDFs with structure-preserving cleanup or an explicit maximum visual mode.", "compress", "compress", "available", ["application/pdf"], ["application/pdf"], "Balanced and lossless browser compression preserve selectable text, links, form fields, vectors, and page structure. Maximum mode flattens pages only when explicitly selected. The app only offers a download when the result is smaller."],
  ...
  ["request-signatures", "Request Signatures", "Assign required fields and route a PDF through an ordered, verified signing workflow.", "sign", "send", "available", ["application/pdf"], ["application/pdf"], "Signed-in owners can route a PDF through up to ten ordered recipients, assign fields per signer, require the invited Google account email and explicit signing consent, track completion and server timestamps, open manual reminder drafts, revoke active requests, and download the current or final PDF plus a SHA-256 activity record. Automatic email or SMS delivery, government-ID checks, qualified timestamps, and independent digital certificates are not included."],
