...
const definitions = [
  ...
  ["protect-pdf", "Protect PDF", "Require a password before a PDF can be opened.", "protect", "lock", "partial", ["application/pdf"], ["application/pdf"], "AES-256 password encryption runs locally in your browser. Account recovery, enterprise password policy, certificates, and digital-rights enforcement are not implemented."],
...
export function getRelatedTools(tool) {
  const preferred = tool.relatedTools.map((id) => TOOL_BY_ID.get(id)).filter((candidate) => candidate && candidate.status !== "coming-soon");
  if (preferred.length >= 3) return preferred.slice(0, 3);
  return TOOL_REGISTRY.filter((candidate) => candidate.id !== tool.id && candidate.status !== "coming-soon" && (candidate.category === tool.category || !preferred.includes(candidate))).slice(0, 3);
}
