...
export async function createTemplatePdf(templateId, values, { styleId = "modern", items = [] } = {}) {
  ...
  pdf.setAuthor(safe(values.businessName || values.company || values.name || "PDFEnrich user").slice(0, 120));
  pdf.setCreator("PDFEnrich template builder");
  ...
  if (definition.legal) {
    ...
    page.drawLine({ start: { x: margin, y }, end: { x: 612 - margin, y }, thickness: 1, color: line });
    y -= 15;
    text("Template for review only. PDFEnrich does not provide legal advice. Laws and required terms vary; have qualified counsel review this draft before use or signature.", { size: 7.5, font: italic, color: muted, lineHeight: 10 });
  }

  const pages = pdf.getPages();
  pages.forEach((pdfPage, index) => {
    pdfPage.drawText(`Created privately with PDFEnrich  |  Page ${index + 1} of ${pages.length}`, { x: margin, y: 24, size: 7, font: regular, color: muted });
  });
  return pdf.save();
}
