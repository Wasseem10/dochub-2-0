  ...
  return (
    <main className="tools-directory tools-directory--catalog">
      <PageMetadata
        ...
        description={`Browse ${releasedTools.length} PDFArrow tools for editing, organizing, converting, signing, protecting, scanning, reviewing, and understanding PDFs.`}
        canonicalUrl="/tools"
          ...
          <section className="tools-catalog-featured" aria-labelledby="popular-tools-title">
            ...
            <div className="tools-catalog-card-grid">
              {popularTools.map((tool, index) => (
                <Link key={tool.id} className={`tools-catalog-card is-tone-${index % 8}`} to={tool.route}>
                ...
                <section key={group.id} id={`category-${group.id}`} className="tools-catalog-group">
                  ...
                  <div className="tools-catalog-card-grid">
                    {groupTools.map((tool, index) => (
                      <Link key={tool.id} className={`tools-catalog-card is-tone-${(index + 3) % 8}`} to={tool.route}>
                        <span><ToolIcon name={tool.icon} size={22} /></span>
                        <strong>{tool.name}</strong>
                        <ChevronRight size={16} />
                      </Link>
