...
const pageContent = {
  [ROUTE_PATHS.privacy]: {
    ...
    sections: [
      ...
      ["Account and synchronization features", "Signing in uses Firebase Authentication. When the authenticated account-document API is configured, PDFs opened while signed in automatically sync as finished private PDFs with minimal version metadata for cross-device access. PDFEnrich does not upload workspace state, extracted OCR text, thumbnails, undo history, or the signature library as separate cloud data."],
  ...
  [ROUTE_PATHS.dataRetention]: {
    ...
    sections: [
      ...
      ["Account and cloud documents", "Firebase Authentication keeps the account until you delete it. PDFs opened while signed in sync to account-restricted storage; older browser-only documents are imported when opened. Account deletion requires recent authentication and confirms removal of active account data before removing the sign-in identity; if the cloud purge fails, PDFEnrich stops instead of reporting a partial deletion as complete."],
