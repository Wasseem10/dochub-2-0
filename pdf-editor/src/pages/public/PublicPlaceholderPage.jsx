...
const pageContent = {
  [ROUTE_PATHS.privacy]: {
    ...
    sections: [
      ["Browser processing", "The editor and supported merge, split, organize, rotate, page-removal, PDF-to-image, and image-to-PDF tools process supported files in your browser. Files are not sent to RealPDF merely to run these tools."],
      ...
      ["Product analytics", "RealPDF may record anonymous events such as a tool opening, broad file-size and page-count buckets, validation categories, and whether an export succeeded. It does not put file names, PDF contents, extracted text, document URLs, signatures, form values, or user-entered document data in analytics events."],
      ["Deletion", "Guest documents can be removed by clearing the site's browser storage. Account document deletion removes the document record and attempts to remove its Firebase payload when cloud persistence is configured. RealPDF does not claim an automatic deletion schedule that has not been implemented."],
  ...
  [ROUTE_PATHS.security]: {
    intro: "RealPDF separates browser-only PDF work from optional account and cloud features, and avoids claiming safeguards that have not been verified.",
    sections: [
  ...
  [ROUTE_PATHS.help]: {
    ...
    sections: [
      ...
      ["Download", "Use the editor or tool's download action. RealPDF does not add a watermark and does not require signup before a supported download."],
  ...
  [ROUTE_PATHS.terms]: {
    intro: "Basic terms for the current free RealPDF service.",
    sections: [
      ["Permitted use", "Use RealPDF only with files you are authorized to view and modify, and do not use the service for unlawful activity."],
      ["Free service", "Supported RealPDF tools are currently free. There is no paid plan, checkout, watermark, or promised service level in this version."],
  ...
  [ROUTE_PATHS.pricing]: {
    intro: "RealPDF is currently completely free.",
...
export function PublicPlaceholderPage({ path, title, description, status }) {
  ...
  return <main className="public-info-page">
    <PageMetadata title={`${title} | RealPDF`} description={content?.intro || description} canonicalUrl={path} noIndex={noIndex} />
    <section className="public-info-hero"><span className="route-status-pill">{content ? "RealPDF information" : status}</span><h1>{title}</h1><p>{content?.intro || description}</p><div><Link to={ROUTE_PATHS.tools}>Browse working tools</Link><Link to={ROUTE_PATHS.editPdf}>Choose a PDF</Link></div></section>
    {content ? <div className="public-info-sections">{content.sections.map(([heading, copy]) => <section key={heading}><h2>{heading}</h2><p>{copy}</p></section>)}</div> : <section className="route-state-card"><h2>Not part of the core public product today</h2><p>This route remains available for compatibility, but RealPDF does not present unfinished sales, enterprise, integration, or template features as working product capabilities.</p></section>}
  </main>;
}
