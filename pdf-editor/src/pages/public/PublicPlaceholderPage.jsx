...
const pageContent = {
  [ROUTE_PATHS.privacy]: {
    ...
    sections: [
      ...
      ["Optional Firebase account features", "Signing in uses Firebase Authentication. If Firebase Storage and Firestore are configured, account holders may store document history and document workspace data in Firebase for cross-device access. Ordered signature requests also store PDF versions, owner and recipient emails, assigned fields, signing-consent records, status, server timestamps, reminder counts, and SHA-256 fingerprints. Expiration blocks signer access; revocation removes PDF versions, and dashboard or account deletion removes the remaining owner-controlled record."],
  ...
  [ROUTE_PATHS.security]: {
    ...
    sections: [
      ...
      ["Account boundary", "Dashboard and cloud-history routes require authentication. Signature-request owners control each request; invited signers must use a Google account whose email matches their assigned recipient record. This confirms control of that Google account, not a government identity, phone number, or signing certificate."],
  ...
  [ROUTE_PATHS.dataRetention]: {
    ...
    sections: [
      ...
      ["Signature requests", "An ordered signature request keeps its current PDF and routing record in Firebase. Signer access stops at expiration. Revoking removes stored PDF versions, and the owner can permanently delete completed or revoked routing records from the signature dashboard; account deletion also removes owner-linked requests. Records include participant emails, field assignments, explicit signing consent, status, timestamps, manual reminder counts, and document fingerprints. Previously downloaded copies cannot be retrieved."],
