...
function ComparisonFileCard({ label, record, inputRef, onFile, disabled }) {
  return <div className={`comparison-file-card ${record ? "has-file" : ""}`}>
    ...
    <span>{record ? <FileText size={24} /> : <Upload size={24} />}</span><small>{label}</small><strong>{record?.file.name || "Choose a PDF"}</strong><p>{record ? pageCountLabel(record.pdf.numPages) : "PDF up to 25 MB and 75 pages"}</p>
...
export function ComparePdfPage({ tool }) {
  ...
  return <main className="image-conversion-page comparison-page">
    ...
    <section className="comparison-action-card"><FileDiff size={25} /><div><strong>{first && second ? "Both PDFs are ready" : "Add an original and revised PDF"}</strong><small>{first && second ? `${pageCountLabel(Math.max(first.pdf.numPages, second.pdf.numPages))} will be checked` : "Your files stay on this device"}</small></div><button type="button" disabled={!first || !second || status === "comparing" || status === "exporting"} onClick={compare}>{status === "comparing" ? <><LoaderCircle className="is-spinning" size={18} /> Comparing {progress}%</> : "Compare PDFs"}</button></section>
    {results.length > 0 && <section className="comparison-results"><header><div><small>Comparison complete</small><h2>{changed} of {pageCountLabel(results.length)} changed</h2><p>Highlights group nearby visual changes into reviewable regions. Word counts show text added or removed.</p></div><button type="button" disabled={status === "exporting"} onClick={download}>{status === "exporting" ? <LoaderCircle className="is-spinning" size={18} /> : <Download size={18} />} Download report PDF</button></header><div className="comparison-page-list">{results.map((result) => <article key={result.pageNumber}><div className="comparison-page-summary"><span>Page {result.pageNumber}</span><strong className={`comparison-status is-${result.statusLabel.toLowerCase().replace(" ", "-")}`}>{result.statusLabel}</strong><small>{result.similarity.toFixed(1)}% similar · +{result.textAdded} / -{result.textRemoved} words · {result.rects.length} visual {result.rects.length === 1 ? "region" : "regions"}</small></div><div className="comparison-previews"><figure>{result.firstPreview ? <div><img src={result.firstPreview} alt={`Original page ${result.pageNumber}`} />{result.rects.map((rect, index) => <i className="comparison-change-region" aria-hidden="true" key={index} style={{ left: `${rect.x * 100}%`, top: `${rect.y * 100}%`, width: `${rect.width * 100}%`, height: `${rect.height * 100}%` }} />)}</div> : <div className="comparison-missing">Not present</div>}<figcaption>Original</figcaption></figure><figure>{result.secondPreview ? <div><img src={result.secondPreview} alt={`Revised page ${result.pageNumber}`} />{result.rects.map((rect, index) => <i className="comparison-change-region" aria-hidden="true" key={index} style={{ left: `${rect.x * 100}%`, top: `${rect.y * 100}%`, width: `${rect.width * 100}%`, height: `${rect.height * 100}%` }} />)}</div> : <div className="comparison-missing">Not present</div>}<figcaption>Revised</figcaption></figure></div></article>)}</div></section>}
