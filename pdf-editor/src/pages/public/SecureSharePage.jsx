...
export function SecureSharePage() {
  ...
  if (state.status !== "ready") {
    ...
    return <main className="secure-share-page"><section className="secure-share-state"><Lock size={28} /><h1>{expired ? "This sharing link has expired" : "This sharing link cannot be opened"}</h1><p>{expired ? "Ask the document owner to create a new link." : "The link may be invalid, revoked, or no longer available."}</p><Link to={ROUTE_PATHS.home}>Go to PDFEnrich</Link></section></main>;
  }
  ...
  return (
    <main className="secure-share-page">
      ...
      <footer className="secure-share-footer"><Lock size={14} /> Read-only sharing by PDFEnrich. The owner can revoke this link at any time.</footer>
    </main>
  );
}
