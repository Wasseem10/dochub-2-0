...
export function SupportPage() {
  ...
  return <main className="support-page">
    <PageMetadata title="Support | PDFEnrich" description="Contact PDFEnrich support about product, account, privacy, or security questions." canonicalUrl={ROUTE_PATHS.support} />
    <section className="support-intro"><span><LifeBuoy size={16} /> PDFEnrich support</span><h1>Tell us what went wrong.</h1><p>Send a private request with enough detail to reproduce the problem. Never include passwords, payment details, or confidential PDF content.</p></section>
    <div className="support-layout"><form onSubmit={submit} className="support-form">
      {status === "sent" ? <div className="support-sent"><CheckCircle2 size={34} /><h2>Request received</h2><p>Your message is now in the owner-only support inbox. Keep this page open if you want to send another request.</p><button type="button" onClick={() => { setStatus("idle"); setForm({ name: "", email: "", category: "product_help", message: "" }); }}>Send another request</button></div> : <>
        ...
        <p className="support-privacy-notice">PDFEnrich uses your name, email, category, and message to respond and stores the request for up to 180 days unless it is needed longer for security or legal reasons. <Link to={ROUTE_PATHS.privacy}>Privacy Policy</Link></p>
    ...
    </form><aside><LockKeyhole size={23} /><h2>What is stored</h2><p>Your name, reply email, selected category, message, account ID if signed in, and submission time are stored in Firebase. Only the authorized PDFEnrich operator can open the support inbox.</p><p>Do not attach or paste PDF contents. PDFEnrich support does not need the document itself for an initial report.</p></aside></div>
  </main>;
}
