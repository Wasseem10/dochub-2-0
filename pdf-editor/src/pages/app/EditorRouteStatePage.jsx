  ...
  return (
    <main className="route-state-page">
      <section className="route-state-card" aria-live="polite">
        ...
        <h1>{title}</h1>
        <p>{message}</p>
        {!isLoading && <div className="route-state-actions"><button type="button" onClick={onBack}>{backLabel}</button>{onHome && <button type="button" onClick={onHome}>PDFEnrich home</button>}</div>}
      </section>
    </main>
  );
}
