...
The selected July 30, 2026 homepage “How it works + privacy” redesign is Paper Trail (ideation option 1), saved at `work/design-references/homepage-paper-trail-selected-2026-07-30.png`: replace the generic three-card row with one connected upload-edit-download document journey using the existing professional PDF illustration family, then follow it with an open warm-white trust band, a document-shield asset, and three divided proof statements. Keep the section bright, tactile, and spacious on desktop, with a deliberately vertical connected path on mobile; do not restore generic numbered cards or the large lavender privacy container.

The selected July 30, 2026 homepage FAQ redesign is Guided Questions (refined ideation option 3), saved at `work/design-references/homepage-faq-guided-selected-2026-07-30.png`: use a two-column editorial FAQ with a large left headline and original document-question illustration, a numbered vertical question path on the right, one clearly expanded answer at a time, and the three reassurance statements integrated as one open rail beneath the FAQ. Preserve the warm-white, navy, coral, lilac, yellow, and powder-blue system, and recompose the numbered path into a readable vertical mobile experience; do not restore generic rounded accordion cards or a disconnected reassurance strip.
...
Google authentication must use Firebase redirect sign-in on phones and coarse-pointer mobile devices, with popup sign-in retained for desktop and redirect fallback when the popup is blocked. Production uses the same-origin `/__/auth/handler` callback on both `pdfenrich.com` and `www.pdfenrich.com`, backed by the `/__/auth/*` Firebase Hosting proxy; do not regress to a popup-only flow or expose raw provider errors to users, logs, or analytics.
