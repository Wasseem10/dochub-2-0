...
The selected July 29, 2026 Popular Tools redesign is Professional Document Commands (ideation option 2), with `work/design-references/popular-tools-professional-selected-2026-07-29.png` as the exact visual source. Reproduce its document-outline plus command-mark anatomy using layered production Tabler glyphs with consistent 24px geometry, round joins, and uniform stroke weight; keep file outlines navy and command details PDFEnrich blue, remove pastel icon blobs, and never stretch, warp, crop, or AI-render individual tool icons.
