...
function staticContentFor(record) {
  ...
  if (record.resource) {
    ...
    const sections = `${primaryAction}${workedExample}${resource.sections.map(/** @param {{ heading: string, body: string, bullets?: string[] }} section */ (section) => `<section><h2>${escapeHtml(section.heading)}</h2><p>${escapeHtml(section.body)}</p>${section.bullets ? `<ul>${section.bullets.map(/** @param {string} bullet */ (bullet) => `<li>${escapeHtml(bullet)}</li>`).join("")}</ul>` : ""}</section>`).join("")}`;
