...
const routeRecords = [
  ...
  ...PUBLIC_PLACEHOLDER_ROUTES.filter(({ path }) => path !== ROUTE_PATHS.features && !isEditorialResourcePath(path)).map((route) => ({ ...route, title: `${route.title} | FixThatPDF`, description: legalDescriptions[route.path] || route.description, noIndex: !Object.hasOwn(legalDescriptions, route.path) })),
...
function metadataFor(record) {
  ...
  const socialImageAlt = record.resource ? `${record.resource.title} — original FixThatPDF resource` : record.tool ? `${record.tool.name} — FixThatPDF browser tool and guide` : "FixThatPDF browser PDF workspace";
...
function staticContentFor(record) {
  ...
  if (record.resource) {
    ...
    return `<main class="prerender-shell"><nav aria-label="Breadcrumb"><a href="/resources">Resources</a><span aria-hidden="true"> / </span><span>${escapeHtml(resource.eyebrow)}</span></nav><p class="prerender-brand">FixThatPDF · ${escapeHtml(resource.eyebrow)}</p><h1>${escapeHtml(resource.title)}</h1><p class="prerender-lead">${escapeHtml(resource.lede)}</p><p>Reviewed ${escapeHtml(resource.reviewedLabel)} by ${escapeHtml(resource.owner)}.</p>${sections}${matrixTable}${benchmarkTable}${redactionProof}${downloads}${related}</main>`;
  }
