...
describe("PDFEnrich tool registry", () => {
  ...
  it("exposes every catalog workflow as released with an explicit support contract", () => {
    ...
    expect(counts.available).toHaveLength(61);
    ...
    expect(TOOL_REGISTRY.find((tool) => tool.id === "flatten-pdf")).toMatchObject({ status: "available" });
